...
//  for(auto host:cluster_0_hosts)
  ...
  for(int i=1;i<cluster_0_hosts.size();i++){
	  sg4::Actor::create("actor_0", cluster_0_hosts[0], sender,mail_box);
	  char actor_name[]="";
	  sprintf(actor_name,"%s.%d",cluster_0_hosts[0]->get_cname(),i);
	  sg4::Actor::create(actor_name,cluster_0_hosts[i],receiver,mail_box);}
